...
void thinningIterationNoSub(cv::Mat& img, int iter)
{
...
void thinningNoSub(const cv::Mat& src, cv::Mat& dst)
{
    dst = src.clone();
    ...
    do {
        thinningIterationNoSub(dst, 0);
        thinningIterationNoSub(dst, 1);
...
float extractRGB_chipsNoSub(Mat &img,Mat &mask){
...
MatrixXd getRGBarrayNoSub(Mat img){
    ...
    for(unsigned int i=1;i<23;i++){
        ...
        float b_avg = extractRGB_chipsNoSub(b, cc);
        float g_avg = extractRGB_chipsNoSub(g, cc);
        float r_avg = extractRGB_chipsNoSub(r, cc);
...
void get_standardizationsNoSub(Mat img, float &det, MatrixXd &rh,MatrixXd &gh,MatrixXd &bh){
    //-- Extending source RGB chips to squared and cubic terms
    MatrixXd source1, source2, source3;
    source1 = getRGBarrayNoSub(img);
...
Mat color_homographyNoSub(Mat img, MatrixXd r_coef,MatrixXd g_coef,MatrixXd b_coef){
...
vector<Point> keep_roiNoSub(Mat img,const Point& tl, const Point& br, Mat &mask){
...
vector<double> ComputerVisionNoSub::get_shapesNoSub(const vector<Point>& cc,const Mat& mask){
    //-- Get measurements
    ...
    float fd = get_fdNoSub(mask);
    double oof = is_oofNoSub(mask);
...
Mat ComputerVisionNoSub::drawShapesNoSub(Mat org, const vector<Point>& cc){
...
Mat ComputerVisionNoSub::get_color_HueNoSub(const Mat& img,const Mat& mask){
    Mat composite;
...
Mat ComputerVisionNoSub::get_color_SaturationNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2HSV);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_color_ValueNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2HSV);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_color_RNoSub(const Mat& img,const Mat& mask){
    Mat composite;
...
Mat ComputerVisionNoSub::get_color_GNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2RGB);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_color_BNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2RGB);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_color_LNoSub(const Mat& img,const Mat& mask){
    Mat composite;
...
Mat ComputerVisionNoSub::get_color_GMNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2Lab);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_color_BYNoSub(const Mat& img,const Mat& mask){
    Mat composite;
    cvtColor(img,composite,COLOR_BGR2Lab);
    vector<Mat> channels1;
    split(composite, channels1);
...
Mat ComputerVisionNoSub::get_RGB_HISTNoSub(const Mat& img, const Mat& mask){
  ...
  const cv::Scalar background_color(0,0,0);
    ...
    for (size_t idx=0; idx < 7; idx++)
    {
        ...
        cv::Mat histogram;
        if(idx==0){
            histogram = get_color_RNoSub(img,mask);
          }
        if(idx==1){
            histogram = get_color_GNoSub(img,mask);
          }
        if(idx==2){
            histogram = get_color_BNoSub(img,mask);
          }
        if(idx==3){
            histogram = get_color_GMNoSub(img,mask);
          }
        if(idx==4){
            histogram = get_color_BYNoSub(img,mask);
          }
        if(idx==5){
            histogram = get_color_HueNoSub(img,mask);
          }
        if(idx==6){
            histogram = get_color_SaturationNoSub(img,mask);
          }
        if(idx==7){
            histogram = get_color_ValueNoSub(img,mask);
          }
...
Mat ComputerVisionNoSub::remove_backgroundNoSub(const Mat& img,int b1LM,int b1HM,int b2LM,int b2HM,int x1,
  ...
  if(ColorStandardization==true){
      ...
      get_standardizationsNoSub(img,det,rh,gh,bh);

      adjImg = color_homographyNoSub(img,rh,gh,bh);
  ...
  Mat pot_roi;
  vector<Point> cc_pot = keep_roiNoSub(pot_and,Point(x1,y1),Point(x2,y2),pot_roi);
  ...
  Mat b_roi;
  vector<Point> cc1 = keep_roiNoSub(b_er,Point(x1,y1),Point(x2,y2),b_roi);
  Mat b_dil;
  ...
  Mat mask;
  vector<Point> cc = keep_roiNoSub(b_xor,Point(x1,y2),Point(x1,y2),mask);

  return pot_and;

}

vector<Point> ComputerVisionNoSub::get_ccNoSub(Mat img,int x1, int y1, int x2, int y2){
    Mat temp;
    vector<Point> cc_pot = keep_roiNoSub(std::move(img),Point(x1,y1),Point(x2,y2),temp);
    return cc_pot;

}

vector<int> ComputerVisionNoSub::getTipsNoSub(const Mat &img){
  ...
  medianBlur(img, img, 3);
      ...
      thinningNoSub(thinning_erode,thinning_erode );
...
vector<int> ComputerVisionNoSub::getNodesNoSub(const Mat &img){
